  ...
  getLooks: async ({ id = "", shop = window.lookbook.shop } = {}) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${process.env.REACT_APP_API_SHOPLOOKS_SERVER_URL}/api/get_looks?shop=${shop}&id=${id}`
      );
      set(
        produce((state) => ({
          ...state,
  ...
  postLooks: async ({
    ...
    try {
      const { data } = await axios.post(
        `${process.env.REACT_APP_API_SHOPLOOKS_SERVER_URL}/api/post_looks`,
        {
          shop,
          name,
          price,
  ...
  destroyLooks: async (id) => {
    ...
    try {
      const { data } = await axios.delete(
        `${process.env.REACT_APP_API_SHOPLOOKS_SERVER_URL}/api/destroy_looks?id=${id}`
      );
